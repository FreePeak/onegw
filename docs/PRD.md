...
Pinned by `internal/server/prefixed_model_test.go` (the advertised list carries no doubled id) and the extended `internal/translat` model-mapping test (`cursor/auto` → wire `auto` → `default`).
...
`opencode/union-alpha` — the single target of the `xdev` combo — asked a surface that does not carry the model and
got the upstream's generic `500 {"type":"error","error":{"type":"error","message":"Internal server error"}}`,
which surfaced to the agent as a bare "Internal server error". Live probe 2026-09-17 (both gateway keys, both
tiers) pinned the contract: `/v1/chat/completions` **500**, `/v1/responses` **500**, `/v1/messages` **200** in the
Anthropic shape — and all three of `x-api-key` + `anthropic-version: 2023-06-01` + `x-opencode-session` are
required (Bearer on `/v1/messages` → 401 "Missing API key."; no session → 400 MissingSessionID).
`provider.AnthropicOnlyModel` names the id, `Def.UpstreamFormat`/`Def.Path` route it to `FmtAnthropic` +
`/v1/messages` on both opencode tiers (Responses-only and chat catalogs unchanged), and `applyAuth` now takes the
routed model so the credential follows the wire — OpenCode's Anthropic-only ids go out with `x-api-key` and no
Bearer. `openCodeGoModels` gains `union-alpha` (36 ids, still the live `GET /zen/go/v1/models` list). Tests: the
catalog/routing tables extended, a provider-level `Do` test asserting path + credential headers +
`CallResult.Format`, and an end-to-end server test driving an OpenAI chat client through `union-alpha` into the
translated `/v1/messages` body and back out as an OpenAI reply. **Deploy: operator restarted the gateway on this
binary — verify with one real `xdev` request.**
...
429 with the z.ai weekly pool reading 100% until 09-18), so `free` had exactly ONE live leg and no fallback.
All 21 routable lanes re-screened with a 3-rep fixed-prompt A/B plus `/metrics`, `/admin/api/v1/subscription` and
the request ring. LIVE: b-ai/qwen3.8-flash (2901×200 / 10×502 / 2×504 on 60-250K-token agentic turns, ring
delivered p50 62 tok/s over 264M cache-read tokens — the only lane proven under this box's real load, so it stays
leg 1), kilocode/kilo-auto/free (8/8 200s, decode 134-175 tok/s, TTFT 6.7-8.7s), b-ai/hy3 (4/6, and both misses are
...
are probed (measured: unwarmed `fast` served b-ai at 62 tok/s; after one request per leg it served
opencode/deepseek-v4.1-flash at 144 tok/s). TWO LANDMINES FOR THE NEXT DEPLOY: (1) the LOCAL master chain
(12 ahead / 53 behind origin) still rejects `strategy = "size-aware"` in `config.Validate`
...
tokenrouter leg (504) and served 200 with vendor-counted in=335,933 on glm/glm-5.3-flash (20.4s). Operational
facts from the diagnosis: (1) the ring records NO row for 4xx/terminal upstream attempts — the original
incident left zero trace rows (only 504/499 kinds appear), which delayed RCA; (2) glm-5.3-flash's coding-plan
window is ≥335,933 tokens, so tokenrouter's z-ai/glm-5.3-free (262,144) is the only known small-window leg;
...
account for a drawn length (via the injectable `pickN`), then closes the run and rotates to
the next slot with one round-robin advance. A 429 or cooldown on the running account aborts
its run immediately (`endRunFor`); the cursor still advances one slot so the next
