...
//
// A profile exposes BOTH sign-in paths. The browser flow is what the dashboard
// runs (9router ships only that one). The device flow is the Cline SDK's
// DEFAULT (useWorkOSDeviceAuth ?? true) and the only one usable headless, so it
// is wired too: it reuses the framework's RFC 8628 shape against WorkOS
// user_management and adds Cline's register hop, which is where the device
// login's real difference lives.
...
const (
	...
	// Device-flow endpoints. The Cline extension/CLI signs in through WorkOS
	// user_management with this PUBLIC native-app client id (no secret), and
	// the pair that comes out of it is not the inference credential: it is
	// exchanged at clineRegisterURL, which answers with Cline's own session.
	// Reference: sdk/packages/core/src/auth/cline.ts:275-420
	// (requestWorkOSDeviceAuthorization / pollWorkOSTokens / registerWorkOSTokens).
	clineWorkOSClientID        = "client_01K3A541FN8TA3EPPHTD2325AR"
	clineWorkOSDeviceURL       = "https://api.workos.com/user_management/authorize/device"
	clineWorkOSAuthenticateURL = "https://api.workos.com/user_management/authenticate"
	clineRegisterURL           = "https://api.cline.bot/api/v1/auth/register"
)
...
// clineAuthenticateEndpoint / clineRegisterEndpoint resolve the device flow's
// two hops, falling back to the vendor's URLs. Fields rather than constants so
// a test (or a stand-in) can point the whole flow at its own server.
func (p Provider) clineAuthenticateEndpoint() string {
	if p.ClineAuthenticateURL != "" {
		return p.ClineAuthenticateURL
	}
	return clineWorkOSAuthenticateURL
}

func (p Provider) clineRegisterEndpoint() string {
	if p.ClineRegisterURL != "" {
		return p.ClineRegisterURL
	}
	return clineRegisterURL
}
...
// ---------------------------------------------------------------------------
// Device flow: RFC 8628 against WorkOS, then Cline's register exchange.
// ---------------------------------------------------------------------------

// clineDevicePoller is the two-hop device login. Everything the framework owns
// — the device start, the pending/slow_down/expired/denied sentinel loop, the
// interval bump — stays the generic RFC 8628 implementation, pointed at the
// WorkOS endpoints; Cline's contribution is one POST that trades the
// authorization server's pair for a session the chat API accepts.
type clineDevicePoller struct {
	p  Provider
	hc *http.Client
}

// hops returns the generic poller aimed at this profile's device endpoints.
// TokenURL is swapped because the base profile points it at Cline's code
// exchange, which is the wrong target for a device grant.
func (c clineDevicePoller) hops() rfc8628 {
	w := c.p
	w.TokenURL = c.p.clineAuthenticateEndpoint()
	return rfc8628{p: w, hc: c.hc}
}

func (c clineDevicePoller) Start(ctx context.Context) (*DeviceStart, error) {
	return c.hops().Start(ctx)
}

// Poll returns the Cline session once the user has approved; every "not yet"
// answer belongs to the framework and passes through untouched.
func (c clineDevicePoller) Poll(ctx context.Context, deviceCode string) (*Token, error) {
	pair, err := c.hops().Poll(ctx, deviceCode)
	if err != nil {
		return nil, err
	}
	return c.p.clineRegister(ctx, c.hc, *pair)
}

// clineRegister trades the authorization server's token pair for Cline's own
...
func (p Provider) clineRegister(ctx context.Context, hc *http.Client, pair Token) (*Token, error) {
	...
	status, resp, err := postJSONRaw(ctx, hc, p.clineRegisterEndpoint(), body)
	if err != nil {
		return nil, fmt.Errorf("cline register: %w", err)
	}
	if status >= 400 {
		return nil, fmt.Errorf("cline register: HTTP %d: %s", status, truncate(resp))
	}
	// No fallback refresh on purpose: the authorization server's refresh token
	// cannot renew a Cline session, so carrying it over would only turn the
	// refresher into a stream of doomed POSTs. An empty field makes the store
	// hold a credential that expires and asks for a re-login — the honest
	// answer when the vendor refused to issue one (the SDK treats a missing
	// refresh token as a hard error for the same reason).
	tok, err := clineTokenOf(resp, "")
	if err != nil {
		return nil, fmt.Errorf("cline register: %w", err)
	}
