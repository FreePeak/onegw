...
func TestSearchQueryExtraction(t *testing.T) {
	...
	d := searxDef(srv.URL)
	res, apiErr := d.Do(context.Background(), &d.Accounts[0], "search/query", "", bytes.NewReader(body), false)
	if apiErr != nil {
...
func TestSearchQueryFromPartsContent(t *testing.T) {
	...
	d := searxDef(srv.URL)
	if _, apiErr := d.Do(context.Background(), &d.Accounts[0], "search/query", "", bytes.NewReader(body), false); apiErr != nil {
		t.Fatalf("Do: %v", apiErr)
	}
...
func TestSearchNoUserMessageIs400(t *testing.T) {
	...
	_, apiErr := d.Do(context.Background(), &d.Accounts[0], "search/query", "", bytes.NewReader([]byte(`{"messages":[{"role":"assistant","content":"hi"}]}`)), false)
...
func TestSearchNonStreamingCompletion(t *testing.T) {
	...
	d := searxDef(srv.URL)
	res, apiErr := d.Do(context.Background(), &d.Accounts[0], "search/query", "", bytes.NewReader(chatBody("q")), false)
	if apiErr != nil {
		t.Fatalf("Do: %v", apiErr)
	}
	defer res.Resp.Body.Close()
...
func TestSearchStreamingIsOpenAISSE(t *testing.T) {
	...
	d := searxDef(srv.URL)
	res, apiErr := d.Do(context.Background(), &d.Accounts[0], "search/query", "", bytes.NewReader(chatBody("q")), true)
	if apiErr != nil {
		t.Fatalf("Do: %v", apiErr)
	}
	defer res.Resp.Body.Close()
...
func TestSearchUpstreamDownIsRetryable503(t *testing.T) {
	...
	d := searxDef("http://127.0.0.1:1")
	_, apiErr := d.Do(context.Background(), &d.Accounts[0], "search/query", "", bytes.NewReader(chatBody("q")), false)
...
func TestSearchAuthViaExtraHeaders(t *testing.T) {
	...
	if _, apiErr := d.Do(context.Background(), &d.Accounts[0], "search/query", "", bytes.NewReader(chatBody("q")), false); apiErr != nil {
		t.Fatalf("Do: %v", apiErr)
	}
...
func TestSearchEmptyResultsContent(t *testing.T) {
	...
	defer srv.Close()
	d := searxDef(srv.URL)
	res, apiErr := d.Do(context.Background(), &d.Accounts[0], "search/query", "", bytes.NewReader(chatBody("lonely query")), false)
	if apiErr != nil {
		t.Fatalf("Do: %v", apiErr)
	}
	defer res.Resp.Body.Close()
