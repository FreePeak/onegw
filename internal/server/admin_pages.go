...
func (s *Server) overviewView() *overviewData {
	...
	if s.st != nil {
		if rows, err := s.st.QueryRange(today, today); err ***REMOVED*** nil {
			for _, r := range rows {
...
func (s *Server) chartJSON(from, to string) string {
	...
	if s.st != nil {
		if raw, err := s.st.QueryRange(from, to); err ***REMOVED*** nil {
			for _, r := range raw {
				label := r.Day
				if from ***REMOVED*** to { // single-day window → hourly buckets
					label = r.Hour
				}
				d := ax.Keys[label]
				d[0] += r.Requests
	...
	if from ***REMOVED*** to { // dense 00..23 hour axis
		for h := range 24 {
	...
	for i, d := range ax.Labels {
		var t time.Time
		if from ***REMOVED*** to { // hour label on the current day
			h, _ := strconv.Atoi(d)
