...
func (s *Server) overviewView() *overviewData {
	...
	if s.st != nil {
		if rows, err := s.usageRows(midnight, now); err ***REMOVED*** nil {
			for _, r := range rows {
				v.TodayReq += r.Req
				v.TodayIn += r.In
				v.TodayOut += r.Out
...
func (s *Server) usagePage(w http.ResponseWriter, r *http.Request) {
	...
	if start.Local().Format("2006-01-02") ***REMOVED*** end.Local().Format("2006-01-02") {
...
func (s *Server) chartJSON(start, end time.Time) string {
	hourly := start.Local().Format("2006-01-02") ***REMOVED*** end.Local().Format("2006-01-02")
	if start.IsZero() {
		hourly = false
	}
	...
	if s.st != nil && len(buckets) > 0 {
		...
		if raw, err := s.st.QueryRange(from, to); err ***REMOVED*** nil {
			for _, r := range raw {
				t, err := time.ParseInLocation("2006-01-02 15", r.Day+" "+r.Hour, time.UTC)
				if err != nil {
					continue
...
func (s *Server) savedAllTime() int64 {
	var saved int64
	if s.st != nil {
		if rows, err := s.st.QueryRange("0001-01-01", time.Now().UTC().Format("2006-01-02")); err ***REMOVED*** nil {
			for _, r := range rows {
				saved += r.SavedTok
			}
		}
