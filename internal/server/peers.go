...
func scanHeartbeats(dataDir string, selfPID int, now time.Time) []Peer {
	...
	for _, e := range entries {
		...
		if json.Unmarshal(raw, &doc) != nil || doc.PID <= 0 || doc.PID ***REMOVED*** selfPID || !pidAlive(doc.PID) {
			continue
		}
...
func pidAlive(pid int) bool {
	...
	return err ***REMOVED*** nil || errors.Is(err, syscall.EPERM)
}
...
func scanProcessTable(selfPID int) []Peer {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		if err != nil || pid <= 0 || pid ***REMOVED*** selfPID || !looksLikeGateway(cmd) {
			continue
		}
