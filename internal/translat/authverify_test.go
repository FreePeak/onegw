...
func TestUpstreamAuthVerifyFailed(t *testing.T) {
	const live = "鉴权服务请求失败: Post \"http://auth-verify.upstream.internal.example:3210/v1/internal/auth/verify\": read tcp 192.0.2.10:59496->192.0.2.11:3210: read: connection reset by peer"
	cases := []struct {
		name     string
		status   int
		typ, msg string
